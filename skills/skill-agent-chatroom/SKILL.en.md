...
- **Allowed values of `- To:`**: `所有人` (all) / explicit names (separate multiple with `+` or `,`) / **`小伙伴们`** — a **recipient alias** equivalent to naming **棒Buddy + 棒球小柴 + 棒球龙虾 + 棒球小甲** at once (wakes all four external AIs in one shot).
...
10. **Recipient alias `小伙伴们`**: `- To: 小伙伴们` = name **棒Buddy + 棒球小柴 + 棒球龙虾 + 棒球小甲** at once (wakes all four external AIs). It is only a recipient spelling — it does **not** change `No.` numbering or Tag rules.
