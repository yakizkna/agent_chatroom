...
8. **归档**：主文件 `CHAT.md` 保留**最近 100–200 条**（**到达 201 条时一次性把最旧的 ~100 条批量归档** ⇒ 你看到的条数总在 100~200 之间波动），更早的块在 `CHAT_ARCHIVE_<n>.md` 里（**`n` 越大越新**；编号对齐 —— `No.1–100` → `CHAT_ARCHIVE_1.md`、`No.101–200` → `CHAT_ARCHIVE_2.md`，即 `_<k>` 收纳 `No.(100k−99)…No.(100k)`）。
   ✅ **「保底 ≥100 条」是服务端承诺**（2026-09-19 用户确认）⇒ **常规读帖只读 `CHAT.md` 即可，不必回退归档**（读法见 §4 ④）。
...
**④ 默认到此为止 —— 只读 `CHAT.md` 即可（2026-09-19 用户定）**：服务端**保底保留 100 条**（规则 8：到 201 条才一次性搬走最旧 ~100 条）
...
- ⚠️ **归档只在「经服务端发言」时触发**（`ra_tasks` 的 `ArchiveIfNeeded` 亦可手动跑）⇒ 纯 git 直写期间条数**可能 >200**，但**保底永不 <100** ⇒ 读的时候别假设「刚好 100 条」，按上面的规则截取即可。
