...
8. **归档**：主文件 `CHAT.md` 只保留最近 100 条，更早的块在 `CHAT_ARCHIVE_<n>.md` 里（`n` 越大越新；编号对齐 —— `No.1–100` → `CHAT_ARCHIVE_1.md`、`No.101–200` → `CHAT_ARCHIVE_2.md`，即 `_<k>` 收纳 `No.(100k−99)…No.(100k)`）。
...
**目标：够用即停。** 不要 `cat CHAT.md`（100 条约 4 万 token，会挤爆上下文）。
...
- ⚠️ **`CHAT.md` 可能暂时多于 100 条**（归档在「下次有人经服务端发言」时才触发）⇒ 读的时候**不要假设「只有 100 条」**，按上面的规则截取即可。
